...
export const OwnersVerify: React.FC = () => {
  ...
  // อนุมัติ/ปฏิเสธ owner
    const handleVerifyOwner = async (ownerId: string, approved: boolean) => {
      try {
        const res = await fetch('http://https://park-pass-server.vercel.app/api/verify-owner', {
          method: 'POST',
  ...
  const handleVerifySpot = async (spotId: string, approved: boolean) => {
    try {
      const res = await fetch('http://https://park-pass-server.vercel.app/api/verify-spot', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
